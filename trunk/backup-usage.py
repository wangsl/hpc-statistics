...
def write_to_excel(excel_file, sheet) :
    ...
    sheet.write(i, 4, "Disk charge", style)
    sheet.write(i, 5, "Server", style)
    sheet.write(i, 6, "Disk (GB)", style)
    style = myeasyxf(colour = (i+1)%2, borders='mdtm')
    sheet.write(i, 7, "Charge", style)
    i +=1
    ...
    for pi in usages.keys() :
        ...
        sheet.write_merge(r1=i, c1=4, r2=i+n_files-1, c2=4,
                          label=float("%.2f" % (total_usage*charge_rate)), style=style)
        ...
        for f in usages[pi].servers.keys() :
            ...
            sheet.write(i, 5, f, style)
            sheet.write(i, 6, float("%.2f" % (usages[pi].servers[f])), style)

            style = myeasyxf(colour = (i+1)%2, borders='nntm')
            if i_file == n_files :
                style = myeasyxf(colour = (i+1)%2, borders='nttm')
            sheet.write(i, 7, float("%.2f" % (usages[pi].servers[f]*charge_rate)), style)
    ...
    sheet.write(i, 4, float("%.2f" % (total_disk_usage*charge_rate)), style)
    sheet.write(i, 5, None, style)
    sheet.write(i, 6, float("%.2f" % (total_disk_usage)), style)
    style = myeasyxf(colour = (i+1)%2, borders='dmtm')
    sheet.write(i, 7, float("%.2f" % (total_disk_usage*charge_rate)), style)
...
if __name__ == "__main__" :
    ...
    setup_usages(input_file = "piback")
    print_usages()
    write_to_excel(excel_file = "backup-usage.xls", sheet = "backup usage")
