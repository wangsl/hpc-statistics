...
def create_invoice_for_pi_all(pi, pi_index, sge_users, disk_users) :

    excel_file = replace(pi.name, ' ', '') + '-all.xls'
    ...
    sheet.write_merge(r1=i, c1=3, r2=i, c2=5, style=style, label='Scratch usage')
    ...
    sheet.write(i, 3, style=style, label='cork/GB')
    sheet.write(i, 4, style=style, label='rio/GB')
    style = myeasyxf2(colour=1, borders='tdtt', alignment='right')
    sheet.write(i, 5, style=style, label='oxford/GB')
    ...
    for hpc_mem in hpc_members :
        ...
        if disk_user :
            ...
            if disk_user.cork :
                sheet.write(i, 3, float("%.2f" % (disk_user.cork)), style)
            ...
            if disk_user.rio :
                sheet.write(i, 4, float("%.2f" % (disk_user.rio)), style)
            ...
            if disk_user.oxford :
                sheet.write(i, 5, float("%.2f" % (disk_user.oxford)), style)
    ...
    sheet.write(i, 5, float('%.2f' % scratch_usage), style)

    if pi.shares_usage :
        #print pi.shares_usage
        #for key in pi.shares_usage :
        #    print pi.shares_usage[key]

        i += 2
        style = myeasyxf2(font='bold_black_underline')
        sheet.write(i, 0, 'Shares Usage', style)

        i += 2
        style = myeasyxf2(colour=1, borders='tdtt', alignment='right')
        sheet.write(i, 0, 'Name of Share', style)
        sheet.write(i, 1, 'Data/GB', style)
        sheet.write(i, 2, 'Replication/GB', style)

        j_max = len(pi.shares_usage)
        #print j_max
        ...
        for key in pi.shares_usage :
            j += 1
            i += 1
            style = myeasyxf2(colour=(j-1)%2, borders='nntt', alignment='right')
            if j == j_max :
                style = myeasyxf2(colour=(j-1)%2, borders='nttt', alignment='right')
            folder = pi.shares_usage[key][0]
            data = pi.shares_usage[key][2]
            replication = pi.shares_usage[key][3]
            sheet.write(i, 0, folder, style)
            sheet.write(i, 1, float('%.2f' % data), style)
            sheet.write(i, 2, float('%.2f' % replication), style)
        ...
        style = myeasyxf2(font='bold_black_underline', alignment='right')
        sheet.write(i, 1, float('%.2f' % pi.shares), style)
        sheet.write(i, 2, float('%.2f' % pi.shares_replication), style)
    ...
    sheet.write(i+2, 0, billing_period, style)
    sheet.write(i+2, 1, "Scratch usage", style)
    sheet.write(i+2, 2, float('%.2f' % scratch_usage), style)
    ...
    sheet.write(i+2, 3, float('%.4f' % (scratch_charge_rate)), style)

    style = myeasyxf2(colour = 0, borders='tntt', alignment='right', num_format_str='$#0.00')
    sheet.write(i+1, 4, float('%.2f' % home_charge), style)
    sheet.write(i+2, 4, float('%.2f' % scratch_charge), style)
    ...
    total_charge = total_cpu*cpu_time_charge_per_hour + \
                   ...
                   home_charge + scratch_charge + replication_charge + \
                   pi.shares*home_charge_rate
